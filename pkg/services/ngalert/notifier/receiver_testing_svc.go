...
func (t *ReceiverTestingService) TestByIntegrationUID(ctx context.Context, user identity.Requester, alert Alert, receiverUID string, integrationUID string) (IntegrationTestResult, error) {
	if receiverUID == "" {
		return IntegrationTestResult{}, ErrInvalidIntegrationTest.Errorf("receiver UID is required")
	}
	if integrationUID == "" {
		return IntegrationTestResult{}, ErrInvalidIntegrationTest.Errorf("integration UID is required")
	}
	...
	if i == nil {
		return IntegrationTestResult{}, ErrIntegrationNotFound.Errorf("")
	}
	if err := t.authz.AuthorizeTest(ctx, user, r); err != nil {
		return IntegrationTestResult{}, err
	}
...
func (t *ReceiverTestingService) TestNewReceiverIntegration(ctx context.Context, user identity.Requester, alert Alert, receiverUID string, integration models.Integration) (IntegrationTestResult, error) {
	if integration.UID != "" {
		return IntegrationTestResult{}, ErrInvalidIntegrationTest.Errorf("integration UID must be empty")
	}
	var receiver *models.Receiver
	if receiverUID == "" {
		// Creator can test its own receivers
		err := t.authz.AuthorizeCreate(ctx, user)
		if err != nil {
			return IntegrationTestResult{}, err
		}
	} else {
		err := t.authz.AuthorizeUpdate(ctx, user, &models.Receiver{UID: receiverUID})
		if err != nil {
			return IntegrationTestResult{}, err
		}
		...
		receiver, err = t.receiverSvc.GetReceiver(ctx, receiverUID, false, user)
		if err != nil {
			return IntegrationTestResult{}, err
		}
	}
	return t.testIntegration(ctx, user, alert, receiver, integration)
}
...
func (t *ReceiverTestingService) PatchIntegrationAndTest(ctx context.Context, user identity.Requester, alert Alert, receiverUID string, integration models.Integration, requiredSecrets []string) (IntegrationTestResult, error) {
	if receiverUID == "" || integration.UID == "" {
		return IntegrationTestResult{}, ErrInvalidIntegrationTest.Errorf("receiver UID is required")
	}
...
func (t *ReceiverTestingService) testIntegration(ctx context.Context, user identity.Requester, alert Alert, receiver *models.Receiver, integration models.Integration) (IntegrationTestResult, error) {
	...
	if err != nil {
		return IntegrationTestResult{}, ErrInvalidIntegrationTest.Errorf(err.Error())
	}
	err = integration.Validate(DecryptIntegrationSettings(ctx, t.encryptionService))
	if err != nil {
		return IntegrationTestResult{}, legacy_storage.MakeErrReceiverInvalid(err)
	}
